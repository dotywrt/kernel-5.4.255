...
/* DOTY Linux-5.4 time API compatibility for legacy MediaTek counter code. */
static inline void doty_gettimeofday_compat(struct timeval *tv)
{
...
static int _mtk_mfg_update_counter(void)
{
	...
	if (ret != PMU_NG) {
		...
		cnt = 0;
		doty_gettimeofday_compat(&tv_end);
...
FINISH:
	if (handle) {
		...
		doty_gettimeofday_compat(&tv_start);
