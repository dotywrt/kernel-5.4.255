...
{
	const char __user *name = NULL; /* Linux 5.4: anonymous VMA name unavailable */
...
#ifdef CONFIG_RTC_LIB
		do {
		struct timespec64 __aee_ts;
		ktime_get_real_ts64(&__aee_ts);
		tv.tv_sec = __aee_ts.tv_sec;
		tv.tv_usec = __aee_ts.tv_nsec / 1000;
	} while (0);
