...
static void mtk_iommu_tlb_add_page(struct iommu_iotlb_gather *gather,
...
static const struct iommu_flush_ops mtk_iommu_flush_ops = {
	...
	.tlb_add_page = mtk_iommu_tlb_add_page,
};
			...
			struct mtk_iommu_data *data)
{
	...
	if (data->dram_is_4gb) /* need it ? */
		pgtable->cfg.quirks |= IO_PGTABLE_QUIRK_ARM_MTK_EXT;
			      ...
			      struct iommu_iotlb_gather *gather)
{
	...
	spin_lock_irqsave(&pgtable->pgtlock, flags);
	unmapsz = pgtable->iop->unmap(pgtable->iop, iova, size, gather);
	spin_unlock_irqrestore(&pgtable->pgtlock, flags);
