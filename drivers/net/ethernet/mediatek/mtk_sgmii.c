...
int mtk_sgmii_init(struct mtk_sgmii *ss, struct device_node *r, u32 ana_rgc3)
{
	...
	for (i = 0; i < MTK_MAX_DEVS; i++) {
		...
		if (IS_ERR(ss->regmap[i])) {
			err = PTR_ERR(ss->regmap[i]);
			of_node_put(np);
			return err;
		}

		if (!of_property_read_string(np, "mediatek,physpeed", &str)) {
			if (!strcmp(str, "2500"))
			       ...
			       const struct phylink_link_state *state)
{
	...
	if (state->speed == SPEED_2500 ||
		...
		gen2 = true;
	else if (id == 1 && !get_gmac1_mode() &&
		 (ss->flags[id] & MTK_SGMII_PHYSPEED_2500))
		gen2 = true;
	else if (state->speed == SPEED_UNKNOWN &&
		 (ss->flags[id] & MTK_SGMII_PHYSPEED_2500))
		gen2 = true;
