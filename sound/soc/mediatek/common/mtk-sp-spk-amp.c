...
/* DOTY-MTK-SPK-DAILINK-LINUX54 */
/* Linux 5.4 ASoC uses snd_soc_dai_link component arrays. */
			    ...
			    const struct snd_soc_ops *i2s_ops)
{
	...
	for (i = 0; i < card->num_links; i++) {
		...
		if (spk_dai_link_idx < 0 &&
		    strcmp(dai_link->cpus[0].dai_name, "I2S1") == 0 &&
		    mtk_spk_i2s_out == MTK_SPK_I2S_1) {
			spk_dai_link_idx = i;
		} else if (spk_dai_link_idx < 0 &&
			   strcmp(dai_link->cpus[0].dai_name, "I2S3") == 0 &&
			   mtk_spk_i2s_out == MTK_SPK_I2S_3) {
			spk_dai_link_idx = i;
		} else if (spk_dai_link_idx < 0 &&
			   strcmp(dai_link->cpus[0].dai_name, "I2S5") == 0 &&
			   mtk_spk_i2s_out == MTK_SPK_I2S_5) {
			spk_dai_link_idx = i;
		}

		if (spk_ref_dai_link_idx < 0 &&
		    strcmp(dai_link->cpus[0].dai_name, "I2S0") == 0 &&
		    mtk_spk_i2s_in == MTK_SPK_I2S_0) {
			spk_ref_dai_link_idx = i;
		} else if (spk_ref_dai_link_idx < 0 &&
			   strcmp(dai_link->cpus[0].dai_name, "I2S2") == 0 &&
