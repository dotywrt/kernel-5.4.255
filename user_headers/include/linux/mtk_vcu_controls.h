 ...
 */
struct gce_cmds {
	__u8  cmd[VCODEC_CMDQ_CMD_MAX];
	__u64 addr[VCODEC_CMDQ_CMD_MAX];
	__u64 data[VCODEC_CMDQ_CMD_MAX];
	__u32 mask[VCODEC_CMDQ_CMD_MAX];
	__u32 cmd_cnt;
};
...
/*
 * 32-bit userspace compatibility ABI.
 *
 * Keep this layout unconditional in UAPI: exported userspace ABI must not
 * depend on kernel CONFIG_* symbols.
 */
